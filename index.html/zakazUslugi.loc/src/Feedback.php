...
class Feedback extends Entity{
    public static $bdFields = ['name','phone','feedback','img','status'];
    public static function getBdFields(): array
    {
        return self::$bdFields;
    }
    ...
    {
        $fields['img_file'] = $files['name'];
        $fields['imgFile'] = $files;
        $this->load($fields);
    }
    public function validate(): void{
        ...
        if(empty($this->imgFile['tmp_name'])){
            throw new InvalidArgumentException('Файл не загружен');
        }
        $allowedExtenstion = ['jpg', 'png', 'gif'];
        $extenstion = pathinfo($this->imgFile['name'], PATHINFO_EXTENSION);
